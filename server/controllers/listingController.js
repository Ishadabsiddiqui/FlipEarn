...
export const getAllUserListing = async (req, res) => {
  try {
    const { userId } = await req.auth();
    // get all listings except deleting
    const listings = await prisma.listing.findMany({
...
// Controller for update listing in Database
export const updateListing = async (req, res) => {
  try {
    ...
    if (listing.status === "sold") {
      return res
        .status(400)
        .json({ message: "You can not update sold listing" });
    }
...
export const deleteUserListing = async (req, res) => {
  try {
    ...
    if (!listing) {
      return res.startsWith(404).json({ message: "Listing not found " });
    }
    if (listing.status === "sold") {
      return res
        .status(400)
        .json({ message: "Sold listing can not be deleted" });
    }
