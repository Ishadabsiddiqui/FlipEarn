import { ArrowDownCircleIcon, BanIcon, CheckCircle, Clock, CoinsIcon, DollarSign, Edit, Eye, EyeIcon, EyeOffIcon, LockIcon, Plus, StarIcon, TrashIcon, TrendingUp, Users, Users2, WalletIcon, XCircle } from 'lucide-react';
...
function MyListing() {
    ...
    const [showWihdrawal, setShowWihdrawal] = useState(null)
    ...
    const deleteListing = async (listingId) => {
        try {
            const confirm = window.confirm("Are you sure you want to delete this listings? if credentials are changed, new credentials will be send to your email");
            if (!confirm) return;
    ...
    return (
        <div className='px-6 md:px-16 lg:px-24 xl:px-32 pt-8'>
            ...
            {/* ---------------- Balance Section------------ */}
            <div className='flex flex-col md:flex-row justify-between gap-4 xl:gap-20 p-6 mb-10 bg-white rounded-xl border border-gray-200'>
                {[
                    ...
                    <div onClick={() => item.label === "Available" && setShowWihdrawal(true)} key={index} className='flex flex-1 items-center justify-between p-4 rounded-lg border border-gray-200 cursor-pointer'>
            ...
            {userListings.length === 0 ? (
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3'>
                    {userListings.map((listing) => (
                        <div key={listing.id} className='bg-white rounded-lg border border-gray-200 hover:shadow-lg shadow-gray-200/70 transition-colors'>
                            <div className='p-6'>
                                                    ...
                                                    <div className='invisible group-hover:visible absolute right-0 top-0 pt-4.5 z-10'>
                                                        <div className='bg-white text-gray-600 text-xs rounded border border-gray-200 p-2 px-3'>
                                                            ...
                                                            <button className='text-nowrap'>
                                                                Status :{" "}
                                                                <span className={listing.isCredentialSubmitted ? listing.isCredentialVerified ? listing.isCredentialChanged ? "text-green-600" : "text-indigo-600" : "text-slate-600" : "text-red-600"}>{listing.isCredentialSubmitted ? listing.isCredentialVerified ? listing.isCredentialChanged ? "Changed" : "Verified" : "Submitted" : "Not Submitted"}</span>
                                                            </button>
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                <div className='space-y-4'>
                                    ...
                                    <div className='flex items-center justify-between pt-3 border-t border-gray-200'>
                                        <span className='text-2xl font-bold text-gray-800'>{currency}{listing.price.toLocaleString()}</span>
                                        <div className='flex items-center space-x-2'>
                                            {listing.status !== "sold" && (
            ...
            {showWihdrawal && (
                <WithdrawModal onClose={() => setShowWihdrawal(null)} />
            )}
            {/* ---------------- Footer ----------------- */}
            <div className='bg-white border-t border-gray-200 p-4 text-center mt-28'>
                <p className='text-sm text-gray-500'>
                    @2026 <span className='text-indigo-600'>Sayyed Yazdan Siddiqui</span>. All rights reserved.
                </p>
            </div>
        </div>
    )
