...
const ManageListing = () => {
    ...
    const platforms = ['youtube', 'instagram', 'tiktok', 'facebook', 'linkedin', 'twitter', 'pinterest', 'twitch', 'snapchat', 'discord'];

    const niches = ['lifestyle', 'fitness', 'food', 'travel', 'tech', 'gaming', 'fashion', 'beauty', 'bussiness', 'education', 'entertainment', 'music', 'art', 'sports', 'health', 'finance', 'other'];
    ...
    // Get listing data from edit if `id` is provided (editmode)
    useEffect(() => {
        if (!id) return;
        setIsEditing(true);
                ...
                <form onSubmit={handleSubmit} className='space-y-8'>
                    ...
                    <Section title='Basic Information'>
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                            ...
                            <InputField label="Listing Title *" value={formData.title} placeholder="e.g., Premium Instagram travel account" onChange={(v) => handleInputChange('title', v)} required={true} />
                    ...
                    <Section title='Account Metrics'>
                        ...
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'>
                            ...
                            <InputField label="Primary Audience Country" value={formData.country} placeholder="United State" onChange={(v) => handleInputChange('country', v)} />
                    ...
                    <Section title="Screenshots & Proof">
                        <div className='border-2 border-dashed border-gray-300 rounded-lg p-6 text-center'>
                            ...
                            <p className='text-sm mt-3 text-gray-500'>Upload Screenshots or Proof of account analytics</p>
                        </div>
...
const InputField = ({ label, value, onChange, placeholder, type = 'text', required = false, min = null, max = null }) => (
    <div>
        ...
        <input type={type} min={min} max={max} placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)} className='w-full px-3 py-1.5 text-gray-600 rounded-md border focus:outline-none focus:ring-2 focus:ring-indigo-500 border-gray-300' />
    </div>
)
...
const SelectField = ({ label, options, value, onChange, required = false }) => (
    <div>
        <label className='block text-sm font-medium text-gray-700 mb-2'>{label}</label>
        <select value={value} onChange={(e) => onChange(e.target.value)} className='w-full px-3 py-1.5 text-gray-600 rounded-md border focus:outline-none focus:ring-2 focus:ring-indigo-500 border-gray-300' required={required}>
...
const CheckBoxField = ({ label, checked, onChange, required = false }) => (
    <label className='flex items-center space-x-2 cursor-pointer'>
        <input type="checkbox" checked={checked} onChange={() => onChange(e.target.checked)} className='size-4' required={required} />
...
const TextareaField = ({ label, value, onChange, required = false }) => (
    <div>
        <label className='block text-sm font-medium text-gray-700 mb-2'>{label}</label>
        <textarea rows={5} value={value} onChange={() => onChange(e.target.value)} className='w-full px-3 py-1.5 text-gray-600 rounded-md border focus:outline-none focus:ring-2 focus:ring-indigo-500 border-gray-300' required={required} />
    </div>
)
